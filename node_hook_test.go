...
func TestValidationNodesUnknownHook(t *testing.T) {
	env, e := CreateEnvironment(buildURL(t, "./testdata/yaml/grammar/nodes_unknown_hook.yaml"), map[string]interface{}{})
...
func TestValidationNodesKnownHook(t *testing.T) {
	env, e := CreateEnvironment(buildURL(t, "./testdata/yaml/grammar/nodes_known_hook.yaml"), map[string]interface{}{})
	assert.Nil(t, e)
	vErrs := env.Validate()
